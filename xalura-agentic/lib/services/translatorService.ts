...
export async function translateText(
  ...
  // STUB — REPLACE IN PHASE 4 with: POST /api/tools/translator
  await new Promise((r) => setTimeout(r, 1200));

  const target = LANGUAGES.find((l) => l.code === to);
  const source = LANGUAGES.find((l) => l.code === from);

  return {
    translated: `[${source?.flag || "🌐"} → ${target?.flag || "🌐"}]\n\n${text}\n\n— Translated to ${target?.name || to} (${options.formality} register)`,
